...
export function computeSpritePos(type, sprite, codeBlock) {
    const spriteMap = {
        [GO_TO]: () => {
            ...
            return { ...sprite, ...calculate(val, codeBlock) }
        },
        [GLIDE_GO_TO]: () => {
        ...
        [CHANGE_X_BY]: () => {
            ...
            return { ...sprite, x: sprite.x + Number(x) }
        },
        [SET_X_TO]: () => {
            const [x] = codeBlock.inputValue
            return { ...sprite, x }
        ...
        [MOVE_STEPS]: () => {
            ...
            return { ...sprite, x, y, speed: sprite.speed + speed }
        },
        [TURN_LEFT]: () => {
            const [angle] = codeBlock.inputValue;
            return { ...sprite, ...turnCalculation(-angle, sprite) }
        },
        [TURN_RIGHT]: () => {
            const [angle] = codeBlock.inputValue;
            return { ...sprite, ...turnCalculation(angle, sprite) }
